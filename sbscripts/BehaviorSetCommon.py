import os
from SmartBody import StringVec, VecMap


def createRetargetInstance(scene, srcSkelName, tgtSkelName):
    endJoints = StringVec()
...
def retargetMotion(scene, motionName, srcSkelName, tgtSkelName, outDir) :
    ...
    if existMotion is not None:
        # do nothing if the retargeted motion is already there
        return
    ...
    outMotion = testMotion.retarget(outMotionName, srcSkelName, tgtSkelName,
                                    endJoints, relativeJoints, offsetJoints)
    cleanMotion = testMotion.constrain(outMotionName, srcSkelName, tgtSkelName,
                                       outMotionName, effectorJoints,
                                       effectorRoots)
    saveCommand = 'animation ' + outMotionName
    + ' save ' + outDir
    + outMotionName + '.skm'
